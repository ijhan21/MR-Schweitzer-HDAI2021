...
TRAIN_PATH = './echocardiography/train/A4C/'
VALIDATION_PATH='./echocardiography/validation/A4C/'
# TEST_RATE = 0.2
BATCH_SIZE = 4
NUM_EPOCHS = 10000
LEARNING_RATE = 1e-5
...
folder_name = "runs_a4c/"+d_time.strftime("%Y%m%d%H%M%S")
...
optimizer1 = torch.optim.Adam(unet.parameters(), lr=LEARNING_RATE, betas=(0.0, 0.9), weight_decay=0.0005)
...
for epoch in range(NUM_EPOCHS):
    ...
    writer.add_scalar("SUMMARY_A4C/DSC",DSC,epoch)
    writer.add_scalar("SUMMARY_A4C/JI LOSS",JI,epoch)
    grid_test = torchvision.utils.make_grid(outputs_test_for_grid)
    writer.add_image('PREDICTION_A4C', grid_test, epoch)
    grid_label = torchvision.utils.make_grid(label)
    writer.add_image('LABEL_A4C', grid_label, epoch)
    grid_img = torchvision.utils.make_grid(img)
    writer.add_image('IMAGEA4C', grid_img, epoch)
    if pre_test_loss is None:
        pre_test_loss = test_loss
    if pre_test_loss<test_loss:
        print(str(epoch),' Best A4C DSC:',DSC.item(), 'JI: ', JI.item())
        pre_test_loss= test_loss
